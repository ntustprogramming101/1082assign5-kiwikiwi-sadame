...
boolean isHit(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh){ // Requirement #3
  if(ax + aw > bx &&
     ax < bx + bw &&
     ay + ah > by &&
     ay < by + bh){
...
String convertFramesToTimeString(int frames){	// Requirement #4
  ...
  int timeOfMinutes = floor(seconds / 60);
  int timeOfSeconds = seconds - timeOfMinutes * 60;
  
  String stm = nf(timeOfMinutes, 2);
  String sts = nf(timeOfSeconds, 2);
...
int getEnemyIndexByRow(int row){				// Requirement #6
  for(int i = 0; i < soldierY.length; i++){
    if(row * SOIL_SIZE == soldierY[i]) return i;
  }
		// HINT:
...
void drawCaution(){								// Requirement #6
  ...
  int E = getEnemyIndexByRow(playerRow + 5); 
  if(E != -1){
    image(caution, soldierX[E], soldierY[E] - SOIL_SIZE);
  }
		// HINT:
